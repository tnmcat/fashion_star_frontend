...
function ReviewItemAndShipping() {
    ...
    useEffect(() => {
        if (shippingMethod.length > 0) {
            // Chọn phương thức giao hàng đầu tiên khi component được mount
            const defaultId = shippingMethod[0].id;
            setSelectedShippingMethodId(defaultId);
            dispatch(addShippingmethodId({shippingMethodId: defaultId}));
        }
    }, [shippingMethod, dispatch]);

    const handleRadioChange = (id) => {
        setSelectedShippingMethodId(id);
        dispatch(addShippingmethodId({shippingMethodId: id}));
    };
                    ...
                    <div className="grid grid-cols-3">
                        <div className="col-span-3">
                            <h2 className="text-xl font-bold font-sans">
                                <span className="pr-4">3</span>Review items and
                                shipping
                            </h2>
                        </div>
                    </div>
