...
function ProductDetail() {
    ...
    console.log("reviewAnalyst:  ", reviewAnalyst);
    const variantRender0 = useSelector(selectVariantInfo);
    console.log("variantRender0 -1", variantRender0);
    ...
    useEffect(() => {
        if (variantRender0 && variantRender0.imageDTOList.length > 0) {
    ...
    console.log("Variant Rander 3 ", variantRender);
    console.log("reviewVariantList", reviewVariantList);
    console.log("reviewAnalyst ", reviewAnalyst);
    console.log("variantDTOListForLoop: ", variantDTOListForLoop);
    ...
    const changeValueId = (i, id) => {
        ...
        variantDTOListForLoop.forEach((variant) => {
            const matchedOptionIds = variant.optionValueDTOList?.map(
                (option) => option.id
            );
            const isMatched = idArray.every((id) =>
                matchedOptionIds.includes(id)
            );
            if (isMatched) {
                    ...
                    {reviewAnalyst && (
                        <div className="w-full h-full col-span-2 flex flex-col py-4 border-gray-300 text-bodyFont">
                            ...
                            <div className="flex flex-row items-center text-center text-sm mb-2 ">
                                ...
                                <div>
                                    {reviewAnalyst.summaryDto.rating.toFixed(0)}{" "}
                                    out of 5
                                </div>
                            </div>
                            <span className="text-gray-400 text-xs mb-4">
                                {reviewAnalyst.summaryDto.reviewsTotal} global
                                ratings
                            </span>
                            ...
                            {showRecommend && (
                                <div className="flex flex-col w-10/12 text-start pl-3 font-titleFont text-sm pb-4">
                                    <span>
                                        Customer Reviews, including Product Star
                                        Ratings help customers to learn more
                                        about the product and decide whether it
                                        is the right product for them.
                                    </span>
                                    <br></br>
                                    <span>
                                        To calculate the overall star rating and
                                        percentage breakdown by star, we don’t
                                ...
                                (item, index) => (
                                    <div key={index} className="m-4">
                                        ...
                                        <div className="text-bodyFont text-xs text-gray-500 mb-2">
                                            Variant:
                                            {variantDetail.variantDTOList[0]?.optionValueDTOList.map(
                                                (option, index) => (
                                                    <span key={index}>
                                                        {option.value} |
                                                    </span>
